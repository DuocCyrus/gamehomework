...
```
game/
├── index.html              markup + thứ tự nạp script
├── css/style.css           HUD, menu, cửa hàng, bảng cheat
├── js/
│   ├── 01-core.js          tiện ích, nạp asset, âm thanh WebAudio, input
│   ├── 02-world.js         camera, hệ hạt, dữ liệu ải, nền parallax
│   ├── 03-art-common.js    hàm vẽ dùng chung, boss, đạn
│   ├── 04-art-hero.js      kiếm sĩ kiếm hiệp + khí thuẫn bát quái
│   ├── 05-art-mobs.js      6 loại quái
│   ├── 06-difficulty.js    4 độ khó, cờ cheat, hiểm hoạ Ác Mộng
│   ├── 07-entities.js      nhân vật, quái, boss, đạn, vật phẩm
│   ├── 08-cheat.js         bảng lệnh cheat + phím tắt
│   └── 09-game.js          DOM UI, cửa hàng, quản lý màn, vòng lặp
├── build.mjs               gộp tất cả thành dist/hac-van-kiem.html
└── dist/hac-van-kiem.html  bản một file, tự chứa
```

Các file js dùng chung phạm vi toàn cục và **phải nạp đúng thứ tự** như trong `index.html`
(file sau dùng lại hằng số / lớp của file trước). Đây là script cổ điển, không phải ES module,
nên mở bằng `file://` vẫn chạy được.

### Ghi chú về chống treo

Vòng lặp game được bọc `try/catch` và **luôn gọi lại `requestAnimationFrame`**, nên một ngoại lệ
đơn lẻ không thể làm đứng hình nữa — thay vào đó sẽ hiện bảng “Đã bắt được lỗi” kèm thông báo và
nút *Tiếp tục*. Bảng kết thúc ván dùng bộ đếm chạy theo vòng lặp (không dùng `setTimeout`) và có
thêm một chốt chặn: nếu sau 1,5 giây mà bảng chưa hiện thì hệ thống tự ép hiện. Nhấn `Esc` hoặc
bấm chuột khi đã thắng/thua cũng mở bảng ngay lập tức.

---

## Về asset ngoài internet

Game **mặc định vẽ tay toàn bộ** nên chạy được ngay, không cần mạng, không sợ lỗi CORS khi mở bằng
`file://` hoặc khi nộp bài. Tuy vậy hệ thống nạp sprite sheet vẫn có sẵn ở đầu `js/01-core.js`:

```js
const ASSETS = {
  enabled: true,                                  // ← bật lên
  sheets: {
    hero_idle: { url: 'https://.../Idle.png', frames: 8, fps: 10, anchorY: .86, scale: .55 },
    hero_run:  { url: '.../Run.png',          frames: 8, fps: 14, anchorY: .86, scale: .55 },
    boss_idle: { url: '.../Boss.png',         frames: 8, fps: 8,  anchorY: .92, scale: 1.1 }
  }
};
```

Sheet phải là lưới ngang đều nhau (các frame xếp cạnh nhau trên một hàng). Nguồn miễn phí hợp chủ đề:
**LuizMelo – Martial Hero** (itch.io), **Craftpix free 2D fantasy**, **OpenGameArt**.
Nếu URL lỗi, `img.onerror` sẽ tự động rơi về bản vẽ tay nên game không bao giờ trắng màn.
...
git init
git add .
git commit -m "Hắc Vân Kiếm — web game Canvas/DOM/Events"
git branch -M main
git remote add origin https://github.com/<tên-tài-khoản>/<tên-repo>.git
git push -u origin main
```

Muốn chơi online: vào **Settings → Pages → Source: Deploy from a branch → main / (root)**,
vài phút sau game sẽ chạy tại `https://<tên-tài-khoản>.github.io/<tên-repo>/`.
